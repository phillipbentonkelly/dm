...
<bdc:getHeadlines webObject="${obj}" var="itemHeadline" xsltFile="$configurationURI/Framework/feeds/rss/feed_cleanup.xslt" />
...
<c:if test="${webtype != 'video' and webtype != 'streamtease'}">
	...
	<c:if test="${not empty imageObj}">
		<p:url var="imgSrc" webObject="${imageObj}" public="true" format="image_700w" type="resource" baseDomain="httpCache" />
	</c:if>
	...
	<item>
		...
		<pubDate><p:out value="${pubDate}" format="eomdate|GMT:EE, d MMM yyyy HH:mm:ss" /> GMT</pubDate>
		...
		<c:if test="${not empty imgSrc and fn:trim(imgSrc) != ''}">
			...
			<media:content medium="image" type="image/${imgType}" url="http:${imgSrc}">
		...
		<c:if test="${isGallery eq 'yes'}">
			<x:forEach var="photoGroup" select="$item-story//doc/story/multimedia-container/gallery-container/photogrp" varStatus="loop">
				...
				<media:content medium="image" type="image/${photoFormat}" width="1024" url="http:${photoSrc}">
		...
		<description><![CDATA[<p:out xvalue="$item-story//text/content" xslt="$configurationURI/Framework/feeds/rss/feed_cleanup.xslt" />]]></description>
		<category></category>
	</item>
</c:if>