...
if (!(requestID == null || requestID.isEmpty()) && !(requestHeaderIfMatch == null || requestHeaderIfMatch.isEmpty())) { 
	...
	<c:if test="${not empty obj}">
		...
		<bdc:getMD5Checksum var="storyMD5" content="${storyText}${obj.loid}" />
		<bdc:getMD5Checksum var="metaMD5" content="${metaText}${obj.loid}" />
...
<% if (requestFormat.equals("json")) { %>
	<jp:include url="/SysConfig/WebPortal/BDC/Framework/feeds/placester/data/getHomepageJSON.jsp?mode=${requestMode}&id=${requestID}&JSONCallback=${JSONCallback}"/>
<%}%>
