// Legacy getCookie() function
// Source: bcom-global-scripts.js / bcom_global_scripts.js
// Should only be used by within this file
...
var s = s_gi(s_account) /************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s.charSet = "ISO-8859-1" /* Conversion Config */
s.currencyCode = "USD" /* Link Tracking Config */
s.trackDownloadLinks = true
// disable exit links for events page only
s.trackExternalLinks = window.location.pathname === '/entertainment/events' ? false : true;
s.trackInlineStats = true
s.linkDownloadFileTypes = "exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,mp4,xlsx,docx"
...
// if (!document.referrer.match('/entertainment') && !document.URL.match('/entertainment/events#/') ) {
// 	alert('sending data');
// 	s.usePlugins = true
// } else { alert('not sending data'); }
s.usePlugins = true

function s_doPlugins(s) { /* Add calls to plugins here */
	if (!s.campaign) s.campaign = s.getQueryParam('camp');
	if (!s.campaign) s.campaign = s.getQueryParam('s_campaign');
	if (!s.campaign) s.campaign = s.getQueryParam('sms_cid');
	s.prop2 = s.getQueryParam('rss_id');
	if (!s.prop5) s.prop5 = s.getQueryParam('page');

	// set prop7
	s.prop7 = s.getQueryParam('p1');
	// reassign and pass into plugin
	s.prop7 = s.getValOnce(s.prop7,'s_gvo_p1',0);

	s.prop12 = s.getQueryParam('p12');
	if (!s.eVar1) s.eVar1 = s.getQueryParam('p1');
    // rmk/BDC-1163	
    if (s.events) {
      if (!s.events.match(/,?event2(?!\d)/)) s.events += ",event2";
    } else {
      s.events = "event2";
    }
...
var throughFB = getCookie('fbLogin'),
	loggedInOmniture = getCookie('pathAuth'),
	isBCom_PAS = getCookie('pathAuthSess');
...
s.eVar12 = checkRDBCookie(); /************************** PLUGINS SECTION *************************/
/*
 * Plugin: Set session cookie to enable count of pageviews
 * was used for s_prop3 but not in use now
 */
s.readCookie = new Function("ckname", "var allcookies=document.cookie;var cknameEQ=ckname+'=';var pos=allcookies.indexOf(cknameEQ);" + "if (pos != -1) {var start=pos + cknameEQ.length;var end=allcookies.indexOf(';',start);" + "if (end == -1) end=allcookies.length;return allcookies.substring(start,end);}return '';")
/*
 ...
 */
s.repl = new Function("x", "o", "n", "" + "var i=x.indexOf(o),l=n.length;while(x&&i>=0){x=x.substring(0,i)+n+x." + "substring(i+o.length);i=x.indexOf(o,i+l)}return x"); /** for BCOM-7462 **/
/****************************** MODULES *****************************/
/* Module: Media */
...
s.m_i("Media"); /************* DO NOT ALTER ANYTHING BELOW THIS LINE ! **************/
var s_code = '',
	s_objectID;
