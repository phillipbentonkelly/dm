<xsl:stylesheet version="1.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:html="http://www.w3.org/Profiles/XHTML-transitional"
	xmlns:met="xalan://com.eidosmedia.portal.util.PortalXsltUtil"
	xmlns:p="xalan://com.eidosmedia.portal.util.PortalXsltExtensions"
	xmlns:exsl="http://exslt.org/common"
	extension-element-prefixes="p exsl"
	xmlns:xf="xalan://web.CssXform"
	exclude-result-prefixes="xf html met">
 
	<!-- Commented out due to an issue with the highlight of the search term -->
	<xsl:preserve-space elements="script" />
	<xsl:output method="html" indent="yes" encoding="iso-8859-1" />
	<!-- for servlet start -->
	<xsl:param name="portalPublicUrl" />
	<xsl:param name="portalContext" />
	<xsl:param name="ext" />
	<xsl:param name="baseuri" />
	<xsl:param name="objectPath" />
	<xsl:param name="nextPage" />
	<xsl:param name="lastPage" />
	<xsl:param name="hasTagline" />
	<xsl:param name="storyId" />

	<!-- To break up the story for the new articles -->
	<xsl:param name="startGraf" />
	<xsl:param name="endGraf" />


	<!-- ***** root ****************************** -->
	<xsl:template match="/">
		<xsl:apply-templates select="*" />
	</xsl:template>

	<!-- ****************** remove anything that might be a note,annotation 
		or should otherwise not show up ******************* -->
	<xsl:template match="*[contains(@class,'@notes')]" />
	<xsl:template match="//annotation" />
	<xsl:template match="//headlines" />
	<xsl:template match="//quotes" />
	<xsl:template match="//web-quotes" />
	<xsl:template match="//correction" />
	<xsl:template match="//review-container-art" />
	<xsl:template match="//review-container-book" />
	<xsl:template match="//review-container-cd" />
	<xsl:template match="//review-container-concert" />
	<xsl:template match="//review-container-dance" />
	<xsl:template match="//review-container-dvd" />
	<xsl:template match="//review-container-movie" />
	<xsl:template match="//review-container-restaurant" />
	<xsl:template match="//review-containers" />
	<xsl:template match="//review-container-television" />
	<xsl:template match="//review-container-theater" />

	<!-- ***** html elements ****************************** -->
	<xsl:template
		match="span|div|font|tt|i|b|B|big|small|u|s|strike|em|strong|q|sub|sup|cite|abbr|acronym|hr|blockquote|address|center|ul|dl|ol|li|dt|dd|table|col|colgroup|thead|tfoot|tbody|tr|th|td|br"
		name="html-t">
		<xsl:element name="{name()}">
			<!-- <xsl:call-template name="compute-class-t"/> -->
			<xsl:call-template name="process-attributes-default" />
			<xsl:apply-templates />
		</xsl:element>
	</xsl:template>

	<xsl:template match="content/p">
		<p class="content-text__text">
			<xsl:apply-templates />
		</p>
	</xsl:template>
	
	<xsl:template match="content/p[position() = 3]">
		<p:include uri="$objectPath" styleName="'/SysConfig/WebPortal/BDC/Framework/skins/leaf/common/related_links.jpt'" />
		<p class="content-text__text">
			<xsl:apply-templates />
		</p>
		<p:include uri="$objectPath" styleName="'/SysConfig/WebPortal/BDC/Framework/skins/leaf/common/ads/ad_topright.jpt'" />
	</xsl:template>

	<xsl:template match="content/p[count(//content/p) &lt; 3][position() = last()]"> 
		<p class="content-text__text">
			<xsl:apply-templates />
		</p>
		<p:include uri="$objectPath" styleName="'/SysConfig/WebPortal/BDC/Framework/skins/leaf/common/related_links.jpt'" />
      	<p:include uri="$objectPath" styleName="'/SysConfig/WebPortal/BDC/Framework/skins/leaf/common/ads/ad_topright.jpt'" />
	</xsl:template>

	<!-- **** bold **** -->
	<xsl:template match="b">
		<strong>
			<xsl:apply-templates />
		</strong>
	</xsl:template>

	<!-- **** itals **** -->
	<xsl:template match="i">
		<em>
			<xsl:apply-templates />
		</em>
	</xsl:template>

	<!-- **** anchors **** -->
	<xsl:template match="a">
		<a>
			<xsl:choose>
				<xsl:when
					test="starts-with(@href,'http') or
						substring-after(substring-before(@href,'?'),'.') = 'pdf' or
						substring-after(substring-before(@href,'?'),'.') = 'PDF' or
						substring-after(substring-before(@href,'?'),'.') = 'doc' or 
						substring-after(substring-before(@href,'?'),'.') = 'xls'">
					<xsl:attribute name="target">_blank</xsl:attribute>
					<xsl:attribute name="href">
						<xsl:value-of select="met:digestResourceUrl(0,@href,$portalContext)" />
					</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="href">
						<xsl:value-of select="met:digestPageUrl(0,@href,$portalContext)" />
					</xsl:attribute>
				</xsl:otherwise>
			</xsl:choose>
			<xsl:apply-templates />
		</a>
	</xsl:template>



	<!-- ********* process-attributes-default ************************** -->
	<xsl:template name="process-attributes-default">
		<!-- <xsl:call-template name="compute-class-t"/> -->
		<xsl:for-each select="@*">
			<xsl:choose>
				<xsl:when test="name()='channel' or name()='altid'">
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="{name()}">
						<xsl:value-of select="string()" />
					</xsl:attribute>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:for-each>
	</xsl:template>

	<!-- ***** find and replace template ****************************** -->
	<xsl:template name="string-replace-all">
		<xsl:param name="text" />
		<xsl:param name="replace" />
		<xsl:param name="by" />
		<xsl:choose>
			<xsl:when test="contains($text, $replace)">
				<xsl:value-of select="substring-before($text,$replace)" />
				<xsl:value-of select="$by" />
				<xsl:call-template name="string-replace-all">
					<xsl:with-param name="text"
						select="substring-after($text,$replace)" />
					<xsl:with-param name="replace" select="$replace" />
					<xsl:with-param name="by" select="$by" />
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$text" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>

	<!-- ***** replace all em dashes ****************************** -->
	<xsl:template match="*/text()">
		<xsl:variable name="myVar">
			<xsl:call-template name="string-replace-all">
				<xsl:with-param name="text">
					<xsl:call-template name="string-replace-all">
						<xsl:with-param name="text" select="." />
						<xsl:with-param name="replace" select="' -- '" />
						<xsl:with-param name="by" select="'&#8212;'" />
					</xsl:call-template>
				</xsl:with-param>
				<xsl:with-param name="replace" select="' - '" />
				<xsl:with-param name="by" select="'&#8212;'" />
			</xsl:call-template>
		</xsl:variable>
		<xsl:value-of select="$myVar" />
	</xsl:template>


	<!-- INLINE IMAGES -->
	<xsl:template match="content/photogrp-inline">
		<xsl:variable name="imgSrc" select="photo-inline/@fileref" />
		<xsl:variable name="formattedImage">
			<xsl:value-of
				select="met:digestResourceUrl(0,string($imgSrc),'image_360w',$portalContext)" />
		</xsl:variable>
		<xsl:variable name="formattedImageFullWidth">
			<xsl:value-of
				select="met:digestResourceUrl(0,string($imgSrc),'image_700w',$portalContext)" />
		</xsl:variable>
		<xsl:variable name="inlineCaption" select="normalize-space(captiongrp/caption/p)" />
		<xsl:variable name="inlineCredit" select="normalize-space(captiongrp/credit/p)" />
		<xsl:choose>
			<!-- Left aligned -->
			<xsl:when test="@float='left'">
				<figure class="content-media content-media--inline-left">
					<img src="" class="content-media__image">
						<xsl:attribute name="src"><xsl:value-of
							select="$formattedImage" /></xsl:attribute>
						<xsl:attribute name="alt"><xsl:value-of
							select="captiongrp/alt-tag/p" /></xsl:attribute>
					</img>
					<xsl:if test="$inlineCaption != '' or $inlineCredit != ''">
						<figcaption class="content-media__caption">
							<xsl:value-of select="captiongrp/caption/p" />
							<div class="content-media__credit">
								<xsl:value-of select="captiongrp/credit/p" />
							</div>
						</figcaption>
					</xsl:if>
				</figure>
			</xsl:when>
			<!-- Right aligned -->
			<xsl:when test="@float='right'">
				<figure class="content-media content-media--inline-right">
					<img src="" class="content-media__image">
						<xsl:attribute name="src"><xsl:value-of
							select="$formattedImage" /></xsl:attribute>
						<xsl:attribute name="alt"><xsl:value-of
							select="captiongrp/alt-tag/p" /></xsl:attribute>
					</img>
					<xsl:if test="$inlineCaption != '' or $inlineCredit != ''">
						<figcaption class="content-media__caption">
							<xsl:value-of select="captiongrp/caption/p" />
							<div class="content-media__credit">
								<xsl:value-of select="captiongrp/credit/p" />
							</div>
						</figcaption>
					</xsl:if>
				</figure>
			</xsl:when>
			<!-- Full-width -->
			<xsl:when test="@float='full-width'">
				
				<figure class="content-media content-media--wide">
					<img src="" class="content-media__image">
						<xsl:attribute name="src"><xsl:value-of
							select="$formattedImageFullWidth" /></xsl:attribute>
						<xsl:attribute name="alt"><xsl:value-of
							select="captiongrp/alt-tag/p" /></xsl:attribute>
					</img>
					<xsl:if test="$inlineCaption != '' or $inlineCredit != ''">
						<figcaption class="content-media__caption">
							<xsl:value-of select="captiongrp/caption/p" />
							<div class="content-media__credit">
								<xsl:value-of select="captiongrp/credit/p" />
							</div>
						</figcaption>
					</xsl:if>
				</figure>
			</xsl:when>
			<xsl:otherwise />
		</xsl:choose>
	</xsl:template>
	
	<!-- inline vidya, taken from globe story xslt -->
	<xsl:template match="content/video-container">
		<xsl:if test="string-length(normalize-space(headline/p)) > 0">
			<p>
				<h2 class="subhead"><xsl:value-of select="headline/p"/></h2>
			</p>
		</xsl:if>
		<xsl:variable name="videoId">
			<xsl:value-of select="substring-after(videogrp/video/a/@href,'videoId=')"/>
		</xsl:variable>
		<div>
			<xsl:attribute name="id"><xsl:text>video</xsl:text></xsl:attribute>
			<xsl:attribute name="class"><xsl:text>videoplayer</xsl:text></xsl:attribute>
			<xsl:attribute name="data-schema"><xsl:text>1</xsl:text></xsl:attribute>
			<xsl:attribute name="data-player"><xsl:text>article</xsl:text></xsl:attribute>
			<xsl:attribute name="data-params"><xsl:text>@videoPlayer=</xsl:text><xsl:value-of select="$videoId"/></xsl:attribute>
		</div>
	</xsl:template>
	

	<!-- **** Fragments **** -->
	<xsl:template match="content//fragment">
		<xsl:value-of select="." disable-output-escaping="yes" />
	</xsl:template>
	
	<!-- Added for Twitter cards -->
	<xsl:template match="content//fragment[contains(text(), 'twitter-tweet')]">
		<div class="content-media content-media--embed">
			<xsl:value-of select="." disable-output-escaping="yes" />
		</div>
	</xsl:template>
	
	<!-- Photo captions -->
	<xsl:template match="caption/p">
		<p class="content-text__text">
			<xsl:apply-templates/>
		</p>
	</xsl:template>
	
	<xsl:template match="caption/p[position() = last()]">
		<p class="content-text__text">
			<xsl:apply-templates />&#160;
			<a href="#" class="content-text--gallery__next js-gallery-next" data-prop23="gallery advance - text">[[SLIDE]]</a>
		</p>
	</xsl:template>


	<xsl:template match="tagline/p">
		<xsl:apply-templates />
	</xsl:template>

	<!-- convert an inline-quote into an HTML blockquote -->
	<xsl:template match="inline-quote[not(contains(@class,'@notes'))]">
		<xsl:element name="blockquote">
			<xsl:attribute name="class">content-text__blockquote</xsl:attribute>
			<xsl:if test="source[normalize-space()]">
				<xsl:attribute name="cite"><xsl:value-of select="./source" /></xsl:attribute>
			</xsl:if>
			<xsl:apply-templates />
		</xsl:element>
	</xsl:template>
	<xsl:template match="inline-quote/source" />
	
	<xsl:template match="inline-quote[not(contains(@class,'@notes'))]/p">
		<p class="content-text__blockquote__text">
			<xsl:apply-templates />
		</p>
	</xsl:template>

  	<xsl:template match="pull-quote">
		<p class="content-text__pullquote">
			<xsl:value-of select="./p" />
    	</p>  
  	</xsl:template>
  
	<xsl:template match="p">
		<p><xsl:apply-templates/></p>
	</xsl:template>
	
</xsl:stylesheet>