...
<%@ attribute name="layer" rtexprvalue="true" required="true"%>
...
<%!	
	public String getInheritanceValue(String xvalueFullArg, String finalFolderPathFullArg, int slashCountArg, String layerArg) throws Exception {
  	...
  	if (slashCountArg == 4 || (finalFolderPathFullArg.contains("/Branch/") && slashCountArg == 6)) {
	    ...
	    Transaction transactionHomepage = portalContextHomepage.getCache().activeTransaction();
		
		ObjectId[] idsHomepage = objectProxyHomepageMeta.getCorrelateds(transactionHomepage, layerArg);
		try	{
			String loidHomepage = idsHomepage[0].getStringLoid();
			return loidHomepage;
		}
		catch (Exception e) {
			return "";
		...
		Transaction transaction = portalContext.getCache().activeTransaction();
		
		try {
			ObjectId[] ids = objectProxy2.getCorrelateds(transaction, layerArg);
			String loid = ids[0].getStringLoid();
			return loid;
		} catch (Exception e) {
			...
			String xvalueFullArgNew = xvalueFullArg;
			String layerArgNew = layerArg;
			
			return getInheritanceValue(xvalueFullArgNew, finalFolderPathFullArgNew, slashCountArgNew, layerArgNew);
			}			
		}
	}	
%>
	
<p:out var="dwpLoid" value="" />
...
<%
	...
	String layerArg = (String) jspContext.getAttribute("layer");
	
	int slashCount = StringUtils.countMatches(finalFolderPathFull, "/");
	
	//String lastIndexOfSlash = Integer.toString(sectionStr.lastIndexOf('/'));
	String dwp = getInheritanceValue(xvalueFull, finalFolderPathFull, slashCount, layerArg);
	//out.println("DWP :" + dwp);
	jspContext.setAttribute("dwpLoid",dwp);
%>
	
<p:getObject loid="${dwpLoid}" var="dwpObj" />
<p:out var="currentValue" value="${dwpObj.path}"/>