...
<p:out var="externalCheck" xvalue="$c//video-container/@class"/>

<c:choose>
	<c:when test="${externalCheck eq 'external'}">
		<p:out var="isExternal" value="${1==1}" />
		<p:out var="externalVideoLink" xvalue="$c//video-container[@class='external']/p" />
	</c:when>
	<c:otherwise>
		...
		<c:choose>
			...
			<c:when test="${fn:contains(videoUrl,'performgroup')}">
				<p:out var="videoUrl" xvalue="$c//video-container/videogrp/thumbnail/@fileref" />
			...
			<c:when test="${fn:contains(videoUrl,'cinesport')}">
				<p:out var="videoUrlTemp" xvalue="$c//video-container/@href" />
