...
<p:out var="requiredAssetsPath" value="$configurationURI/required_assets.xml"/>
<p:getObject path="${requiredAssetsPath}" var="requiredAssetsObj"/>
<p:object webObject="${requiredAssetsObj}" content="requiredAssets" />

<p:out var="layer" xvalue="$requiredAssets//requirements[@webType=$webType]/layers/ad" />
<c:if test="${layer == ''}">
	<p:out var="layer" xvalue="$requiredAssets//requirements[@webType='BLANK']/layers/ad" />
</c:if>
