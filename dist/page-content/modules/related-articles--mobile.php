<!-- Related Articles -->
<div class="related-articles--mobile related-articles-modal">
    <h1 class="related-articles__section-header">Related Articles</h1>

    <div class="related-articles__item--mobile">
        <img class="thumb--mobile" src="images/related-articles/related-articles__thumbs__1.jpg" />
        
        <a href="#">
            <h2 class="title--mobile">Tom and Gisele's Brookline Pad Looks Almost Ready</h2>
        </a>

        <p class="description--mobile">The ream home of the most famouse couple in Massachusetts looks like it's close to completion.</p>
        
        <div class="additional-info--mobile">
            <div class="date">September 9, 2014</div>

            <div class="tags">
                <a href="javascript:;" class="category-tag maroon">Luxury</a>
                <a href="javascript:;" class="category-tag orange">Brookline</a>
            </div>
        </div>
    </div>

    <div class="related-articles__item--mobile">
        <img class="thumb---mobile" src="images/related-articles/related-articles__thumbs__2.jpg" />
        
        <a href="#">
            <h2 class="title--mobile">For Sale: $2.85M Italianate Mansion in JP</h2>
        </a>
        
        <p class="description--mobile">This updated Italianate mansion, once known as The Gilbert House, was originally built in 1854.</p>
        
        <div class="additional-info--mobile">
            <div class="date">July 30, 2014</div>

            <div class="tags">
                <a href="javascript:;" class="category-tag light-blue">Open House</a>
                <a href="javascript:;" class="category-tag maroon">Luxury</a>
                <a href="javascript:;" class="category-tag orange">Jaimaca Plain</a>
            </div>
        </div>
    </div>

    <div class="related-articles__item--mobile">
        <img class="thumb---mobile" src="images/related-articles/related-articles__thumbs__3.jpg" />

        <a href='#'>
            <h2 class="title--mobile">For Sale: Historic Weston mansion</h2>
        </a>
        
        <p class="description--mobile">The Henderson House in Weston, an English-Tudor style mansion with nearly 17,000 square feet.</p>
        
        <div class="additional-info--mobile">
            <div class="date">July 13, 2014</div>
        </div>
    </div>

    <div class="related-articles__item--mobile">
        <img class="thumb---mobile" src="images/related-articles/related-articles__thumbs__4.jpg" />
        
        <a href="#">
            <h2 class="title--mobile">New Construction, Chestnut Hill Colonial - $2,400,000</h2>
        </a>

        <p class="description--mobile">Walk to Heath School and Train. Gorgeous Tree-lined street, Established Neighborhood.</p>
        
        <div class="additional-info--mobile">
            <div class="date">July 3, 2014</div>

            <div class="tags">
                <a href="javascript:;" class="category-tag light-blue">Open House</a>
                <a href="javascript:;" class="category-tag maroon">Luxury</a>
            </div>
        </div>
    </div>

    <div class="dfp-ad__row">
        <div id="ad_bigbox2" class="dfp-ad__row__main-link"><!--<img src="images/ads/dfp/dfpAds__300by250__marriott.jpg" />--></div>
        <div id="ad_bigbox3" class="dfp-ad__row__main-link"><!--<img src="images/ads/dfp/dfpAds__300by250__verizon.jpg" />--></div>

        <script>
            var networkCode = 61381659;
            var topLevelAdUnit = "testboston.com";
            var s1 = "testboston.com";
            var s2 = "real-estate";
            var adUnit = topLevelAdUnit + "/" + s2;
            var slotName = "/" + networkCode + "/" + adUnit;
            
            googletag.cmd.push(function() {
                googletag.defineSlot(slotName, [300,250], "ad_bigbox2")
                googletag.defineSlot(slotName, [300,250], "ad_bigbox3")
                .addService(googletag.pubads())
                .setTargeting("pos", "ad_bigbox2")
                .setTargeting("pos", "ad_bigbox3");
                googletag.pubads().setTargeting("s1",s1);
                googletag.pubads().setTargeting("s2",s2);      
                googletag.pubads().enableSingleRequest();
                googletag.enableServices();
            });
        </script>

        <script type="text/javascript">
            googletag.cmd.push(function() {
                googletag.display("ad_bigbox2");
                googletag.display("ad_bigbox3");
            });
        </script>
    </div>

    <div class="related-articles__item--mobile">
        <img class="thumb---mobile" src="images/related-articles/related-articles__thumbs__1.jpg" />
        
        <a href="#">
            <h2 class="title--mobile">Tom and Gisele's Brookline Pad Looks Almost Ready</h2>
        </a>

        <p class="description--mobile">The ream home of the most famouse couple in Massachusetts looks like it's close to completion.</p>
        
        <div class="additional-info--mobile">
            <div class="date">September 9, 2014</div>

            <div class="tags">
                <a href="javascript:;" class="category-tag maroon">Luxury</a>
                <a href="javascript:;" class="category-tag orange">Brookline</a>
            </div>
        </div>
    </div>

    <div class="related-articles__item--mobile">
        <img class="thumb---mobile" src="images/related-articles/related-articles__thumbs__2.jpg" />
        
        <a href="#">
            <h2 class="title--mobile">For Sale: $2.85M Italianate Mansion in JP</h2>
        </a>

        <p class="description--mobile">This updated Italianate mansion, once known as The Gilbert House, was originally built in 1854.</p>
        
        <div class="additional-info--mobile">
            <div class="date">July 30, 2014</div>

            <div class="tags">
                <a href="javascript:;" class="category-tag light-house">Open House</a>
                <a href="javascript:;" class="category-tag maroon">Luxury</a>
                <a href="javascript:;" class="category-tag orange">Jaimaca Plain</a>
            </div>
        </div>
    </div>

    <a href="javascript:;" class="view-more-articles-mobile">View More <img src="images/view-more__arrow-down.gif" /></a>
</div>
<!-- Related Articles -->